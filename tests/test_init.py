...
async def test_async_setup_entry(hass: HomeAssistant, mock_coordinator: MagicMock) -> None:
    entry = MockConfigEntry(domain=DOMAIN, data={"username": "test"}, entry_id="test-id")
...
async def test_update_listener(hass: HomeAssistant, mock_coordinator: MagicMock) -> None:
...
async def test_setup_entry_fires_notification_when_version_absent(hass: HomeAssistant, mock_coordinator: MagicMock) -> None:
    """No CONF_LAST_RATE_SCHEMA_VERSION in entry.data → notification fired."""
    entry = MockConfigEntry(domain=DOMAIN, data={"username": "test"}, entry_id="notify-id")
    entry.add_to_hass(hass)
    with (
        patch(
            "custom_components.dominionsc.DominionSCCoordinator",
        ...
        patch("custom_components.dominionsc.persistent_notification.async_create") as mock_notify,
    ):
...
async def test_setup_entry_no_notification_when_version_current(hass: HomeAssistant, mock_coordinator: MagicMock) -> None:
